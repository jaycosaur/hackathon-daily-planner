...
const StatusBubble: React.FC<{ text: string, otherText: string }> = ({text, otherText}) => {
  return (
    <ListItem alignItems="flex-start">
      <ListItemText
        primary={<>{otherText}</>}
...
export const TaskActivityCard: React.FC<ITaskActivityCardProps> = ({
  ...
  if (taskActivity.type === ETaskActivityType.StatusUpdate) {
    // aaaaa deadline
    return <StatusBubble text={taskActivity.text} otherText={(taskActivity as any).otherText} />;
  }
